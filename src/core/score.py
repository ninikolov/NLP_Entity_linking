    ...
    for query in parser.query_array:
        ...
        for match in query.search_matches:
            ...
            for true_match in query.true_entities:
                ...
                if (match.entities[0].link == true_match.entities[0].link):
...
def print_F1(parser):
    ...
    print("-"*60,"\n{0:<15} | {1:12} | {2:12} | {3}{4:12}{5}".format("LAZY",
     round(precision_l, 4),  round(recall_l, 4), TermColor.BOLD, round(f1_l, 4),  TermColor.END))
    print("{0:<15} | {1:12} | {2:12} | {3}{4:12}{5}".format("STRICT",
     round(precision_s, 4), round(recall_s, 4), TermColor.BOLD, round(f1_s, 4),  TermColor.END))   
    print("*"*60)
